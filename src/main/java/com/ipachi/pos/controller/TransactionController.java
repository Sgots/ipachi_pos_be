...
public class TransactionController {
    ...
    @GetMapping("/combined")
    public ResponseEntity<ApiResponse<List<CombinedTxnDto>>> searchCombined(
            @RequestParam(value = "businessId", required = false) Long businessIdParam,
            @RequestHeader(value = "X-Business-Id", required = false) Long businessIdHeader,
            @RequestParam(value = "sku", required = false) String sku,
            @RequestParam(value = "name", required = false) String name,
        ...
        var grouped = service.searchCombinedByBusiness(businessId, sku, name, txId, dateFrom, dateTo, minQty, maxQty);
        return ResponseEntity.ok(new ApiResponse<>(1, "Success", grouped));
    }
}
